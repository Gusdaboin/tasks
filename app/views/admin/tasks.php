    ...
    <div class="col-md-12 mx-auto text-center">
      <div class="tarjeta shadow-sm">
        <div class="tarjeta-body">
          <a href="/admin/task/create" class="btn-outline-gold float-right shadow-sm my-3">Crear</a>
                  ...
                  <?php if ($facade->include_var('tasks')[0]): ?>
                    <?php foreach ($facade->include_var('tasks') as $task): ?>
                      <tr class="text-light text-center">
                        ...
                        <td>
                          <form action="/admin/task/trash" method="post">
                            <?=$csrf_i;?>
                            <input type="hidden" name="_method" value="DELETE">
                            <input type="hidden" name="id_task" value="<?=$task['id'];?>">
                            <a href="/admin/task/<?=$task['id'];?>" class="btn-link text-black"><i class="ion-ios-eye"></i></a>
                            <a href="/admin/task/<?=$task['id'];?>/edit" class="btn-link text-black"><i class="ion-edit"></i></a>
                    ...
                    <?php endforeach; ?>
                  <?php else: ?>
                    <h1 class="text-center text-black">Aún no hay tareas creadas</h1>
                  <?php endif; ?>
                </tbody>
              </table>
            </div>
